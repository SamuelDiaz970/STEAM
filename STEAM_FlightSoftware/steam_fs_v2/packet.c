...
void packet_head(int buff,
  ...
  int LEN){
    ...
    if(buff == 0){
        ...
        count = buffer_write_hk(FILL2 | TIME_QUALITY << 2 | USEC << 4, count);
    ...
    if(buff == 1){
        count = buffer_write_hr(VER | TYPE << 3| SHDR << 4 | APID << 5, count);
        count = buffer_write_hr(APID >> 3, count);
        count = buffer_write_hr(GRP | SEQ << 2, count);
        count = buffer_write_hr(SEQ >> 6, count);
        ...
        count = buffer_write_hr(FILL2 | TIME_QUALITY << 2 | USEC << 4, count);
	...
	if(buff == 2){
        count = buffer_write_sr(VER | TYPE << 3| SHDR << 4 | APID << 5, count);
        count = buffer_write_sr(APID >> 3, count);
        count = buffer_write_sr(GRP | SEQ << 2, count);
        count = buffer_write_sr(SEQ >> 6, count);
        ...
        count = buffer_write_sr(FILL2 | TIME_QUALITY << 2 | USEC << 4, count);
...
void payload_science_dummy_packet_sr(){
	int buff = 2;
	packet_head(buff, 1, science_packet_apid, sr_packet_count, sr_buff_length);
    sr_packet_count++;

    int packet_tail_location = 16;
	packet_tail_location = buffer_write_sr(0x01,packet_tail_location);//X123_Indicator;//HXR(0x1) or SXR (0x2)
...
void payload_science_dummy_packet_hr(){
	int buff = 1;
	packet_head(buff,1, science_packet_apid, hr_packet_count, hr_buff_length);
    hr_packet_count++;
...
void payload_HK_dummy_packet(){
	int buff = 0;
	packet_head(buff,1, housekeeping_packet_apid, hk_packet_count, hk_buff_length);
    hk_packet_count++;
