...
#define NOOP                        0x00
#define NOOP_LENGTH                 18
#define SET_STEAM_TIME              0x06
#define SET_STEAM_TIME_LENGTH       25


void command_handle(char *command_buf){
    int command = 0;
    command = ((command_buf[0] & 0b11100000)>>5) <<8 | command_buf[1];
    switch(command){
        case NOOP:
            noop();
            break;
...
// 	sleep(1);
// 	get_rtc_time(&time);
// 	strftime(timestamp, 26, "%Y-%m-%d %H:%M:%S", &time);
...
void set_steam_time(char *buffer){
    ...
    int time_sec = 0;
    int time_usec = 0;
    time_sec = buffer[17]<<24 | buffer[18] <<16 | buffer[19] <<8 | buffer[20];
    time_usec = buffer[21] <<16 | buffer[22] <<8 | buffer[23];
    // time_sec = 
    
    itv.it_interval.tv_sec = 1;
	itv.it_interval.tv_usec= 0;
	itv.it_value.tv_sec = time_sec;//1;
	itv.it_value.tv_usec= time_usec;//0;
	setitimer(ITIMER_REAL, &itv, NULL);


    // struct timeval tv;
    // printf("Time before set sec: %d\n", tv.tv_sec);
    // printf("Time before set usec: %d\n", tv.tv_usec);
