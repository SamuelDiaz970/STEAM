...
typedef struct steam_xr_param
{
	...
	unsigned long time_at_boot;
	unsigned long time_last_hxr;
	unsigned long time_last_sxr;
	unsigned long time_last_hk;
	unsigned long ref_time_TAI;
...
void timer_systimer_update(void);
...
int steam_timestamp_init(void)
{
	...
	g_param.time_at_boot = rawtime;
	g_param.time_from_boot = g_param.time_at_boot;
